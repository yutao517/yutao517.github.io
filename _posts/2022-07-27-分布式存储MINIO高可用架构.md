...
```
![在这里插入图片描述](https://img-blog.csdnimg.cn/9879a33cf4bf464ea699ac8e7254eadc.png)
先运行一次minio.sh，因为没有挂载肯定错误，我们做的只是为了创建挂载目录
...
```bash
mkfs.ext4 /dev/sdb #格式化目录
mount /dev/sdb /mnt/data1/ #挂载目录
mkfs.ext4 /dev/sdc
